...
import {buy, getAllItems, getBasket} from "../api";
...
const Shop = ({user}) => {
    ...
    const handlePay = async () => {
        ...
        let itemsInBasket = 0;
        basket.filter((obj) => itemsInBasket += obj.quantity);

        buy(itemsInBasket).then((response) => {
            setBasket(response.data.basket);
            setAllItems(response.data.shop)
        }).catch((err) => {
            setBasket(err.response.data.basket);
            setAllItems(err.response.data.shop);
                    ...
                    <div className="bg-white py-4 px-4 shadow-xl rounded-lg max-w-3xl">
                        <div className="text-lg text-center font-light">
                            <p>If you are part of this community, you obviously know about our SUPERSECRET UglyCon,
                                which takes place every 23rd weekend of the year in a lovely venue in Edinburgh (EH78
                                5HU).
                                And you obviously know that we use a unique currency to buy merch or pay for services
                                at our events: <b>uglyCOINS</b>. Their number is limited, and you can only buy them
                                online.
                                You will
                ...
                <div className="grid grid-cols-12 gap-6">
                    <div className="col-span-12 sm:col-span-12 md:col-span-7 lg:col-span-8 xxl:col-span-8">
                        ...
                        {user.privilege === 'admin' ?
                            <ShopAddItem allItems={allItems} setAllItems={setAllItems}/> : <></>}
